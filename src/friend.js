...
const { getFriendConfig } = require('./userConfig');
 ...
 * 检查好友是否在黑名单中
 */
function isInBlacklist(gid, name) {
    const friendConfig = getFriendConfig();
    const blacklist = friendConfig.blacklist || [];
    
    // 检查是否禁用所有好友巡查
    if (friendConfig.disableAllFriendCheck === true) {
        return true;
    }
    
    // 检查黑名单中是否有 'all'
    if (blacklist.includes('all') || blacklist.includes('ALL')) {
        return true;
    }
    
    // 检查 GID
    if (blacklist.includes(gid)) return true;
    
    // 检查名字（不区分大小写）
    const nameLower = name.toLowerCase();
    for (const item of blacklist) {
        if (typeof item === 'string' && item.toLowerCase() === nameLower) {
            return true;
        }
    }
    return false;
...
async function checkFriends() {
    ...
    try {
        ...
        for (const f of friends) {
            ...
            // 检查黑名单
            if (isInBlacklist(gid, name)) {
                const showDebug = DEBUG_FRIEND_LANDS === true || DEBUG_FRIEND_LANDS === name;
                if (showDebug) {
                    console.log(`[调试] 好友 [${name}] (GID:${gid}) 在黑名单中，跳过`);
                }
                continue;
            }

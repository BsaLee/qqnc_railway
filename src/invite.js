...
const { getInviteConfig } = require('./userConfig');
...
function readInviteLinks() {
    ...
    const links = inviteConfig.links || [];
    ...
    for (const link of links) {
        // 跳过注释和空行
        if (!link || typeof link !== 'string' || link.trim().startsWith('示例:')) {
            continue;
        }
...
async function processInviteCodes() {
    ...
    // 优先从配置文件读取
    let invites = readInviteLinks();
    
    // 如果配置文件没有，尝试从 share.txt 读取（兼容旧方式）
    if (invites.length === 0) {

...
function sendWeComNotification(content) {
    ...
    const url = new URL(notifyConfig.wecomWebhook);
    const options = {
    ...
    const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
            if (res.statusCode === 200) {
                ...
                logWarn('通知', `企业微信消息发送失败: ${res.statusCode}`);
            }
        });
    });

    req.on('error', (e) => {
        logWarn('通知', `企业微信消息发送失败: ${e.message}`);
    });
    ...
    sendMsg('gamepb.userpb.UserService', 'Login', body, (err, bodyBytes, meta) => {
        ...
        try {
            ...
            if (reply.basic) {
                ...
                const loginTime = reply.time_now_millis 
                    ? new Date(toNum(reply.time_now_millis)).toLocaleString('zh-CN', { timeZone: 'Asia/Shanghai' })

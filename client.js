...
async function main() {
    ...
    // 如果命令行没有指定平台，使用配置文件中的平台
    if (!args.includes('--wx') && !args.includes('--qq')) {
        if (loginConfig.platform === 'wx') {
            CONFIG.platform = 'wx';
        } else {
            CONFIG.platform = 'qq';
        }
    ...
    // 如果命令行没有指定 code，尝试从配置文件读取
    if (!options.code) {
        if (CONFIG.platform === 'wx' && loginConfig.wxCode) {
            options.code = loginConfig.wxCode;
            console.log('[配置] 使用配置文件中的微信 code');
        } else if (CONFIG.platform === 'qq' && loginConfig.qqCode) {
            options.code = loginConfig.qqCode;
            console.log('[配置] 使用配置文件中的 QQ code');
        }
    }
    ...
    if (!options.code) {
        if (CONFIG.platform === 'wx') {
            console.log('[参数] 微信模式需要通过 --code 传入登录凭证或在 config.json 中配置 wxCode');
        } else {
            console.log('[参数] 请通过 --code 传入登录凭证或在 config.json 中配置 qqCode');
        }
        showHelp();
        process.exit(1);
    }
